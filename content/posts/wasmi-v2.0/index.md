...
## Differences and Similarities: Wasmi 2.0, Wasm3 and Stitch

Wasmi 2.0, Wasm3 and Stitch have pretty similar interpreter architectures overall.
The table below acts as a quick summary and overview of the similarities and differences.

| | Wasmi 2.0 | Wasm3 | Stitch |
|:--|:--|:--|:--|
| Stack Slots | `u32` or<br>`u16` if `indirect-dispatch` | `usize` | `usize` |
| Handler Signatures | 9 parameters<br>(6-7 GPRs) | 5 parameters<br>(4 GPRs) | 8 parameters<br>(6 GPRs) |
| Accumulator Registers | `ireg`: integers<br>`freg32`: `f32`<br>`freg64`: `f64` | `_r0`: integers<br>`_fp0`: floats | `Ix`: integers<br>`Sx`: `f32`<br>`Dx`: `f64` |
| `(memory 0)` Registers | `mem0_ptr`: data pointer <br> `mem0_len`: length | `_mem`: pointer to memory header | `Md`: data pointer <br> `Ms`: length |
...
- **Bytecode Association:** Wasmi's bytecode is associated to the Wasm module.
  Each instance of the same Wasm module shares the same underlying Wasmi bytecode.
  The internal bytecodes for Wasm3 and Stitch are associated to a Wasm instance,
...
- **Branch Targets:** Wasmi 2.0 uses 32-bit branch offsets which require the executor
  to compute the branch target via `add`. In contrast, Wasm3 and Stitch precompute
  absolute branch targets during compilation of Wasm bytecode to their internal IR.
  Experiments conducted on Wasmi 2.0 showed mostly noise and even a few regressions
  when precomputing branch targets, so that optimization was dropped.
- **Stack Slot Canonicalization:** Wasmi 2.0 widens `i32` and `u32` values to
  `i64` and `u64` values respectively before storing them into 64-bit stack slots (cells).
  Wasm3 and Stitch only write to the lower 32-bits of their 64-bit cells.
  This allows Wasmi 2.0 to drop some instructions that are needed by Wasm3 and Stitch
